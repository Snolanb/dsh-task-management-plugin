...
The repository now includes the initial worker registry, provider/workspace preflight, headless process launcher, lease-aware dispatcher, and tests on the feature/worker-dispatch branch. Autonomous scheduling, production profiles, durable run records, and session-backed model tiers remain to be implemented.
