...
function routeError(error) {
  ...
  if (code === 'TASK_NOT_FOUND') return { status: 404, body: { error: error.message, code } }
  if (code === 'INVALID_TRANSITION' || code === 'TASK_EXISTS' || code === 'DEPENDENCY_CYCLE' || code === 'PARENT_CYCLE' || code === 'LEASE_OWNER_MISMATCH' || code === 'LEASE_EXPIRED' || code === 'TASK_NOT_CLAIMED' || code === 'TASK_NOT_RUNNING' || code === 'MAX_ATTEMPTS_EXCEEDED') return { status: 409, body: { error: error.message, code } }
