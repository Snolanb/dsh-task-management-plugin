...
const CSS = [
  ...
  '.dsh-to-launcher{position:fixed;z-index:10050;right:18px;bottom:18px;border:1px solid var(--dsw-alias-border-l2,#3b3e46);border-radius:18px;background:var(--dsw-alias-bg-layer-2,#25272d);color:var(--dsw-alias-label-primary,#f5f6f7);box-shadow:0 4px 18px #0005;cursor:pointer;padding:7px 13px;font:inherit;font-size:12px}',
  '.dsh-to-launcher:hover{background:var(--dsw-alias-interactive-bg-hover,#343740)}',
  '.dsh-to-panel{position:fixed;z-index:10040;right:16px;bottom:60px;width:min(1180px,calc(100vw - 32px));height:min(760px,calc(100vh - 92px));display:flex;flex-direction:column;overflow:hidden;border:1px solid var(--dsw-alias-border-l2,#3b3e46);border-radius:14px;background:var(--dsw-alias-bg-layer-1,#1c1e23);color:var(--dsw-alias-label-primary,#f5f6f7);box-shadow:0 12px 44px #0008}',
...
export class TaskBoardView {
  ...
  buildShell() {
    ...
    this.statusFilter = node('select', { className: 'dsh-to-select', onChange: event => { this.filterStatus = event.target.value; this.renderBoard() } })
    ...
    this.profileFilter = node('input', { className: 'dsh-to-input dsh-to-input-small', placeholder: 'worker profile', onInput: event => { this.filterProfile = event.target.value; this.renderBoard() } })
    ...
    this.toolbar = node('div', { className: 'dsh-to-toolbar' }, [node('span', { className: 'dsh-to-muted', text: 'Filter' }), this.statusFilter, this.profileFilter, node('span', { className: 'dsh-to-muted', text: 'Worker' }), this.workerInput])
  ...
  async refresh() {
    try {
      ...
      this.renderBoard(); this.renderDetail(); this.setNotice(this.tasks.length + ' tasks')
  ...
  visibleTasks() { return this.tasks.filter(task => (this.filterStatus === '' || task.status === this.filterStatus) && (this.filterProfile === '' || task.worker_profile === this.filterProfile)) }
  ...
  renderCard(task) {
    const card = node('article', { className: 'dsh-to-card' + (task.id === this.selectedId ? ' dsh-to-card-selected' : ''), onClick: () => { this.selectedId = task.id; this.renderBoard(); this.renderDetail() } })
    const meta = []
    ...
    if (task.status === 'running' && task.claimed_by === this.worker) actions.append(button('Complete', () => { this.selectedId = task.id; this.renderDetail() }))
  ...
  renderDetail() {
    ...
    if (task === undefined) { this.detail.append(node('div', { className: 'dsh-to-muted', text: 'Select a task to inspect it, or create a new task.' })); return }
    this.detail.append(node('h3', { text: task.title }), node('div', { className: 'dsh-to-muted', text: task.id + ' · ' + STATUS_LABELS[task.status] }), node('p', { className: 'dsh-to-muted', text: task.description || 'No description.' }))
  ...
  appendDependencies(task) {
    ...
    const form = node('form', { className: 'dsh-to-form dsh-to-section', onSubmit: event => { event.preventDefault(); if (dependencyInput.value) void this.run(() => this.client.addDependency(task.id, dependencyInput.value), task.id) } }, [node('h4', { text: 'Dependencies' }), node('div', { className: 'dsh-to-muted', text: task.blocked_by?.length ? 'Blocked by: ' + task.blocked_by.join(', ') : 'No unfinished blockers.' }), node('div', { className: 'dsh-to-form-row' }, [dependencyInput, node('button', { type: 'submit', className: 'dsh-to-button', text: 'Add' })])])
    this.detail.append(form)
  }
  ...
  showCreate() {
    ...
    const form = node('form', { className: 'dsh-to-form', onSubmit: event => { event.preventDefault(); void this.run(async () => { const task = await this.client.create({ title: title.value, description: description.value, status: status.value, worker_profile: profile.value || undefined, acceptance_criteria: criteria.value.split('\\n').map(value => value.trim()).filter(Boolean) }); this.selectedId = task.id; return task }, undefined, true) } }, [node('h3', { text: 'New task' }), field('Title', title), field('Description', description), field('Initial status', status), field('Worker profile', profile), field('Acceptance criteria', criteria), node('button', { type: 'submit', className: 'dsh-to-button dsh-to-button-primary', text: 'Create task' })])
    this.detail.append(form)
  }
